package loncha.proyectomensajeria.modelo;

import java.security.MessageDigest;

import javax.crypto.Cipher;
import javax.crypto.spec.SecretKeySpec;

public class TransformacionDatos {
	
	public byte[] cifrarPassword(String password) throws Exception {
		final byte[] bytes = password.getBytes("UTF-8");
		final Cipher aes = obtieneCipher(true);
		final byte[] passwordCifrada = aes.doFinal(bytes);
		
		return passwordCifrada;
	}
	
	public String descifrarPassword(byte[] passwordCifrada) throws Exception {
		final Cipher aes = obtieneCipher(false);
		final byte[] bytes = aes.doFinal(passwordCifrada);
		final String passwordDescifrada = new String(bytes, "UTF-8");
		
		return passwordDescifrada;
	}
	
	private Cipher obtieneCipher(boolean paraCifrar) throws Exception {
		final String frase = "FraseLargaConDiferentesLetrasNumerosYCaracteresEspeciales_·¡È…ÌÕÛ”˙⁄¸‹Ò—1234567890!#%$&()=%_NO_USAR_ESTA_FRASE!_";
		final MessageDigest digest = MessageDigest.getInstance("SHA");
		digest.update(frase.getBytes("UTF-8"));
		final SecretKeySpec key = new SecretKeySpec(digest.digest(), 0, 16, "AES");

		final Cipher aes = Cipher.getInstance("AES/ECB/PKCS5Padding");
		if (paraCifrar) {
			aes.init(Cipher.ENCRYPT_MODE, key);
		} else {
			aes.init(Cipher.DECRYPT_MODE, key);
		}

		return aes;
	}
}
